...
def xor(b1, b2=None):
    # (1引数)リスト内のバイト列を文字毎全部xor
    # (2引数)2つのバイト列を文字毎全部xor
...
def encrypt(txt, key, iv):
    ...
    for block in blocks:
        """
            enc_i = enc(plain_i ^ curr_i)
            enc_i ^ plain_i = curr_i+1
        """
        ctxt += aes.encrypt(xor(block, curr))
        curr = xor(ctxt[-bs:], block)  # 暗号ブロックと平文のxor -> 次のcurr
    iv2 = AES.new(key2, AES.MODE_ECB).decrypt(iv2)
    key2 = xor(to_blocks(ctxt))  # 次のkeyは確定できる
    return str(base64.b64encode(iv+ctxt), encoding='utf8')  # このセッションのivは確定できる
