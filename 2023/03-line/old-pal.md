...
- Japanese Ver: <https://project-euphoria.dev/blog/40-line-ctf-2023/#old-pal>

We were given the following perl script.
...
The goal is to send a password that satisfies `eval("$pw == 20230325")` without being filtered out by many constraints.

I noticed that `__LINE__` is allowed, and `eval("__LINE__")` returns 1 (not the line `eval` is executed). So I send `20230326-__LINE__` as password.
